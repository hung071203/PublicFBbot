...
module.exports.handleReply = async function (api, event, client, hdr) {
    ...
    let userMN = await client.apis['database'].getBank(event.threadID, event.senderID)
    ...
    switch (args[0]) {
        ...
        case 'run':
            ...
            if (r1 == r2 && r2 == r3) {
                ...
                for (const e of checkPs) {
                    try {
                      let find = await client.apis['database'].getBank(event.threadID, e.ID);
                      find.money += e.money * 10;
                      client.apis['database'].addBank(find);
                      msg += `${find.name} nhận ${(e.money * 10).toLocaleString('en-US') }$\n`;
                    } catch (error) {
                      console.error(`Lỗi khi cập nhật tiền cho người dùng với ID ${e.ID}:`, error);
                    }
                  }
                const msgs = {
                    body: msg,
                ...
                for (const e of checkPs) {
                    const result = await checkTXiu(client, taixiu, checkTX, e);
                    msg += result;
                }
...
async function checkTXiu(client, taixiu, checkTX, e){
    let find = await client.apis['database'].getBank(event.threadID, e.ID)
    if(e.tx != taixiu){
        find.money -= e.money
        client.apis['database'].addBank(find)
