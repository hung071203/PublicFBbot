...
module.exports.run = async function (api, event, args, client) {
    ...
    if (args.length == 1) {
        ...
        switch (args[1]) {
            case 'help':
                api.sendMessage(`!rule: hiện nội quy nhóm\n!rule set (Nội quy): thêm lại nội quy cho nhóm!\n !rule update: caạp nhật nội quy nhóm`, event.threadID, event.messageID)
                break;
            case 'set':
                let findQTV = await client.apis['database'].getUserThread(event.threadID, event.senderID);
                if(!findQTV?.isQTV) return api.sendMessage('M định làm gì:)? Chỉ quản trị viên mới dùng dc chức năng này!',event.threadID, event.messageID)
                ...
                break;
            case 'update':
                let QTV = await client.apis['database'].getUserThread(event.threadID, event.senderID);
                if(!QTV?.isQTV) return api.sendMessage('M định làm gì:)? Chỉ quản trị viên mới dùng dc chức năng này!',event.threadID, event.messageID)
