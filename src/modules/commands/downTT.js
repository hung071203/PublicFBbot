...
module.exports.handleReaction = async function (api, event, client, hdr) {
  
  if(event.messageID != hdr.messageID) return
  if(event.reaction != '❤') return
  const filename = path.basename(hdr.linkMp3) + '.mp3';
  const filePath = path.join(__dirname, '..', '..', 'img', filename);
  ...
  client.handleReaction = client.handleReaction.filter(item => item.messageID != hdr.messageID)
};
